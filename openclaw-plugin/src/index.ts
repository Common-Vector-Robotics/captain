...
function resolveConfig(pluginConfig: Record<string, unknown> | undefined): CaptainRemoteConfig {
  ...
  for (const key of Object.keys(supplied)) {
    if (!CONFIG_KEYS.has(key)) throw new TypeError(`Unknown Captain remote config field: ${key}.`);
  }
...
async function closeResources(api: OpenClawPluginApi, resources: RuntimeResources): Promise<void> {
  try {
...
function registerRuntime(api: OpenClawPluginApi): void {
