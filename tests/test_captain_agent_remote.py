...
def remote_state_for_env(path, env):
    ...
    return RemoteClientState(
        path, profile_id=remote_profile_id(config), env=env
    )
...
def scripted_server(responses):
    """Serve scripted HTTP responses and record real wire requests."""
...
def test_report_turn_id_is_stable_for_the_same_payload(tmp_path):
    """A retry must reuse its initial report turn, not duplicate it."""
...
def test_changed_initial_payload_is_a_conflict(tmp_path):
    """A report ID must never map to two differently digested turns."""
...
def test_report_turn_is_stable_after_reopening_the_state_store(tmp_path):
    ...
    first = RemoteClientState(path).get_or_create_report_turn(
        "report-1", "digest-1"
    )

    second = RemoteClientState(path).get_or_create_report_turn(
        "report-1", "digest-1"
    )
...
def test_reply_turn_is_stable_for_parent_and_exact_user_text(tmp_path):
    """Retrying a reply must reuse its turn while its question is pending."""

    state = RemoteClientState(tmp_path / "remote.sqlite3")
    state.replace_pending("report-1", "parent-turn", ["Ship Friday?"])

    first = state.get_or_create_reply_turn(
        "report-1", "parent-turn", "reply-digest"
    )
    second = state.get_or_create_reply_turn(
        "report-1", "parent-turn", "reply-digest"
    )
...
def test_changed_reply_text_digest_gets_a_new_turn_without_replacing_pending(
    tmp_path,
):
    """A corrected reply is distinct but cannot alter its Captain question."""

    state = RemoteClientState(tmp_path / "remote.sqlite3")
    state.replace_pending("report-1", "parent-turn", ["Ship Friday?"])

    first = state.get_or_create_reply_turn(
        "report-1", "parent-turn", "digest-yes"
    )
    second = state.get_or_create_reply_turn(
        "report-1", "parent-turn", "digest-no"
    )
...
def test_reply_for_stale_parent_fails_without_creating_a_turn(tmp_path):
    """A prior question cannot accept a reply once a newer one replaces it."""
...
def test_pending_replacement_is_durable_and_questions_are_immutable(tmp_path):
    """The question context survives reopen and exposes no mutable list."""
...
def test_pending_question_value_validates_and_freezes_direct_construction():
    """The pending value cannot be forged with empty or mutable questions."""

    pending = PendingCaptainQuestions(
        "report-1", "parent-turn", ["Exact question?"]
    )
...
def test_pending_replacement_atomically_changes_parent_and_questions(tmp_path):
    """A new question context replaces both fields as one current state."""
...
def test_clearing_pending_is_report_scoped_and_idempotent(tmp_path):
    """Finishing one report cannot erase another's unanswered question."""
...
def test_pending_requires_a_nonempty_bounded_sequence_of_strings(
    tmp_path, questions
):
    """Invalid question context must not persist for a later continuation."""
...
def test_get_pending_ignores_malformed_persisted_json(tmp_path):
    """A corrupt question row must fail closed, not crash or guess context."""
    ...
    with sqlite3.connect(path) as connection:
        connection.execute(
            "UPDATE pending_questions SET questions_json = ? "
            "WHERE report_id = ?",
            ("{not-json", "report-1"),
        )
    ...
    assert state.get_pending("report-1") is None
    with pytest.raises(RemoteStateConflict):
        state.get_or_create_reply_turn(
            "report-1", "parent-turn", "reply-digest"
        )
...
def test_remote_profile_id_is_one_way_stable_and_configuration_scoped():
    """Rotation keeps one member profile; origin or member changes isolate."""

    first = RemoteConfig("https://captain.example/", MEMBER_ONE_TOKEN)
    equivalent = RemoteConfig(
        "https://captain.example", MEMBER_ONE_ROTATED_TOKEN
    )
...
def test_nondefault_remote_ports_remain_distinct_profiles(
    ...
    assert remote_profile_id(default_config) != remote_profile_id(
        nondefault_config
    )


def test_turn_and_pending_context_continue_across_member_token_rotation(
    tmp_path,
):
    """A replacement secret for one member reopens the same durable profile."""
    ...
    rotated = RemoteClientState(
        path, profile_id=remote_profile_id(rotated_config)
    )
...
def test_turns_and_pending_questions_are_namespaced_by_remote_profile(tmp_path):
    """Equal report IDs on two remotes must not replay or answer each other."""
    ...
    second_turn = second.get_or_create_report_turn(
        "shared-report", "same-digest"
    )
    ...
    assert (
        first.get_or_create_report_turn("shared-report", "same-digest")
        == first_turn
    )
    assert (
        second.get_or_create_report_turn("shared-report", "same-digest")
        == second_turn
    )
...
def test_existing_unscoped_state_is_migrated_but_never_claimed_by_a_profile(
    tmp_path,
):
    """A v1 upgrade must quarantine legacy rows, not guess their ownership."""
    ...
    scoped_turn = scoped.get_or_create_report_turn(
        "shared-report", "current-digest"
    )
    assert scoped_turn != legacy_turn
    assert (
        scoped.get_or_create_report_turn("shared-report", "current-digest")
        == scoped_turn
    )
...
def test_profile_scoped_database_contains_no_raw_origin_or_credential(tmp_path):
    """Durable retry state retains only the one-way remote profile ID."""
...
def test_remote_state_path_honors_exact_override_then_xdg_default(
    monkeypatch, tmp_path
):
    """Remote state follows the report-store path rules with its own file."""

    monkeypatch.setenv("HOME", str(tmp_path / "home"))

    exact = str(tmp_path / " remote state.sqlite3 ")
    assert (
        remote_state_path({"CAPTAIN_REMOTE_STATE_PATH": exact}) == Path(exact)
    )
    assert remote_state_path({"CAPTAIN_REMOTE_STATE_PATH": "   "}) == (
        tmp_path / "home" / ".local" / "state" / "captain-agent"
        / "remote.sqlite3"
    )
...
def test_state_initialization_preserves_an_existing_custom_parent_mode(
    tmp_path,
):
...
def test_relative_state_path_preserves_the_current_directory_mode(
    monkeypatch, tmp_path
):
    """A relative override must not re-mode the cwd or its shared parent."""
    ...
    assert (
        stat.S_IMODE((shared_parent / "remote.sqlite3").stat().st_mode)
        == 0o600
    )


def test_default_dedicated_parent_is_remoded_when_reopened(
    monkeypatch, tmp_path
):
    """The normal state directory remains owner-only across process reopen."""
...
def test_symlinked_state_parent_is_rejected_without_touching_its_target(
    tmp_path,
):
    """A symlinked parent cannot redirect local state elsewhere."""
...
def test_symlinked_state_database_is_rejected_without_touching_its_target(
    tmp_path,
):
...
@pytest.mark.parametrize(
    "bad_turn_id", ["not-a-uuid", sqlite3.Binary(b"not-a-uuid")]
)
...
def test_blob_report_id_candidate_fails_closed_without_a_second_turn(
    tmp_path, turn_kind
):
    """A non-text report ID must not hide a corrupt initial or reply row."""
    ...
    with sqlite3.connect(path) as connection:
        assert connection.execute(
            "SELECT COUNT(*) FROM remote_turns WHERE turn_kind = ?",
            (turn_kind,),
        ).fetchone()[0] == 1


def test_corrupt_initial_parent_fails_closed_without_a_second_turn(tmp_path):
    """An initial row with a parent is corrupt, not a new reservation."""
...
def test_corrupt_reply_row_fails_closed_instead_of_creating_around_it(
    ...
    """A malformed reply row cannot be bypassed by a second retry turn."""
...
def test_database_contains_only_allowed_remote_state_fields(tmp_path):
    """The database excludes credentials, URLs, report bodies, and replies."""
    ...
    with sqlite3.connect(path) as connection:
        ...
        stored_text = "\n".join(
            row[0]
            for row in connection.execute(
                "SELECT turn_id FROM remote_turns UNION ALL "
                "SELECT questions_json FROM pending_questions"
            )
        )

    for forbidden in (
        "token", "url", "body", "metadata", "reply", "email", "identity",
    ):
...
def test_submit_uses_exact_path_body_and_one_private_authorization_header():
    """The wire request carries the token once in a header, never the URL."""
...
def test_remote_client_ignores_ambient_http_and_https_proxies(monkeypatch):
    """Credentials must go directly to the configured target, never a proxy."""
    ...
    ) as (target_url, target_requests, _):
        with scripted_server(
...
def test_submit_disconnect_is_unknown_and_never_reposted():
    """A lost submit response is ambiguous; the server may have queued it."""
...
def test_poll_conflict_and_size_rejection_are_definitive_failed(status):
    """A definitive poll rejection must not become an ambiguous outcome."""
...
def test_in_flight_poll_timeout_uses_only_remaining_deadline_budget():
    """A poll socket timeout shrinks so no request crosses the deadline."""
...
def test_submit_429_waits_then_reposts_the_identical_stable_turn():
    """An explicit rejection may retry only the same idempotent POST."""
...
def test_submit_429_deadline_expires_failed_without_sleep_or_repost():
    """A turn never accepted by its deadline is a definitive busy failure."""
...
def test_submit_429_then_disconnect_never_retries_the_ambiguous_post():
    """Only explicit rejection retries; a lost retry response is ambiguous."""
...
def test_retry_after_that_crosses_deadline_returns_queued_without_extra_poll():
    """An excessive valid delay must stop at the deadline, not oversleep."""
...
def test_submit_http_statuses_map_without_reflecting_remote_body(
    status, expected
):
    """Public errors separate safe rejection from ambiguous server receipt."""

    turn_id = "b73db2fe-ec74-4f44-a74c-fbe44eb11e46"
    with scripted_server(
        [{"status": status, "body": "remote-secret-body"}]
    ) as (url, _requests, _script):
        result = remote_client(url).submit_and_poll(
            "report-1",
            turn_id,
            {"turn_id": turn_id, "kind": "reply", "reply": "Yes"},
...
def test_mismatched_or_malformed_envelope_is_unknown_without_polling(
    bad_envelope,
):
    """The client must never follow IDs supplied by an invalid envelope."""

    turn_id = "b73db2fe-ec74-4f44-a74c-fbe44eb11e46"
    with scripted_server(
        [{"status": 202, "body": bad_envelope}]
    ) as (url, requests, _script):
        result = remote_client(url).submit_and_poll(
            "report-1",
            turn_id,
...
def test_malformed_or_oversized_response_is_unknown_and_not_reflected(
    response_body,
):
...
def test_local_mode_delegates_to_existing_handler_with_exact_arguments(
    monkeypatch,
):
    """No remote variables preserves the local call boundary unchanged."""
    ...
    result = handle_captain_turn(
        "report-1", VALID_REPORT, {"client": "pytest"}, env=environment
    )
    assert result is sentinel
    assert calls == [
        ("report-1", VALID_REPORT, {"client": "pytest"}, environment)
    ]


def test_partial_remote_config_never_calls_local_or_network(monkeypatch):
    """A half-configured client fails closed before choosing a transport."""
...
def test_invalid_remote_report_does_not_create_continuation_state(tmp_path):
    """Validation must finish before any durable or network side effect."""
...
def test_oversized_remote_reply_preserves_pending_without_reserving_turn(
    tmp_path,
):
    ...
    assert result.status == "failed"
    assert requests == []
    assert remote_state_for_env(state_path, env).get_pending(
        "report-1"
    ).questions == ("Ship Friday?",)
    with sqlite3.connect(state_path) as connection:
        assert connection.execute(
            "SELECT COUNT(*) FROM remote_turns WHERE turn_kind = 'reply'"
        ).fetchone()[0] == 0
...
def test_submit_429_deadline_failure_preserves_pending_reply(
    tmp_path, monkeypatch
):
    """A never-accepted reply leaves its parent available for retry."""
...
def test_remote_report_updates_pending_only_from_terminal_response(tmp_path):
    """A terminal Captain question becomes the parent for a later reply."""
...
def test_remote_reply_preserves_exact_text_and_replaces_pending(tmp_path):
    """A reply body has only the exact user text and its generated turn."""
    ...
    with scripted_server([]) as (url, requests, script):
        ...
        reply_turn = state.get_or_create_reply_turn(
            "report-1", "parent-turn", reply_digest
        )
...
def test_remote_terminal_reply_without_questions_clears_pending(tmp_path):
    """A completed continuation removes only its answered question context."""
...
def test_queued_reply_keeps_pending_context(tmp_path):
    ...
    try:
        RemoteCaptainClient.submit_and_poll = (
            lambda *_args, **_kwargs: monkeypatch_result
        )
        result = handle_captain_turn("report-1", reply="Yes", env=env)
    finally:
...
def test_terminal_envelope_with_queued_public_result_keeps_pending(tmp_path):
    """A queued public result is not completion evidence for continuation."""
...
def test_reply_without_pending_and_mixed_report_reply_fail_before_io(tmp_path):
    """Continuation dispatch needs exactly one valid input and a parent."""
    ...
    with scripted_server([]) as (url, requests, _script):
        ...
        mixed = handle_captain_turn(
            "report-1", VALID_REPORT, {}, "Yes", env=env
        )
...
def test_switching_remote_credential_cannot_replay_or_answer_same_report(
    tmp_path,
):
    """A token change makes a new turn and hides the prior pending reply."""
    ...
    with scripted_server([]) as (url, requests, script):
        ...
        second = handle_captain_turn(
            "report-1", VALID_REPORT, {}, env=second_env
        )
    ...
    assert len(requests) == 2
    assert (
        remote_state_for_env(state_path, first_env).get_pending("report-1")
        is not None
    )
    assert (
        remote_state_for_env(state_path, second_env).get_pending("report-1")
        is None
    )


def test_local_cancellation_clears_only_the_applicable_profile_without_http(
    tmp_path,
):
    """A refusal clears local pending state without sending it as a reply."""
...
def test_cancellation_rejects_mixed_content_without_clearing_or_sending(
    tmp_path,
):
    """Cancellation is a local action, never a report or reply modifier."""
    ...
    assert requests == []
    assert (
        remote_state_for_env(state_path, env).get_pending("report-1")
        is not None
    )


def test_local_reply_requires_remote_mode_without_calling_local(monkeypatch):
    """Local replay remains report-only until a remote continuation exists."""

    monkeypatch.setattr(
        "captain_agent.dispatch.handle_session_report",
        lambda *_args, **_kwargs: pytest.fail(
            "local report handler was called"
        ),
    )
