...
3. If `shadow`: DRY RUN — the wrap is never posted to `program_channel`; it is sent only
   ...
   `critical_paths.py write-state`, and `memory/daily/` when that private overlay exists)
   is still written for real in
...
4. If `live`: the wrap posts for real to `program_channel` and ClickUp writes execute for
   real.
...
1. ClickUp writes in this cron are autonomous and audited: in `live` audience, apply
   ...
   (live: posted to `program_channel`; shadow: in the `shadow_recipient` preview) with the
...
2. `message` tool only; no employee DMs from this cron, with exactly one named exception:
   ...
   any id listed in `excluded_user_ids`, in either audience.
...
1. Work from the installed Captain workspace root (the directory containing `CLAW.md`).
   Read `MEMORY.md` when it exists; its absence is valid.
2. Load `.secrets/clickup.env`; fetch EOD board read-only: `scripts/fetch_clickup_tasks.py
   ...
   `program_channel` post; shadow: in the `shadow_recipient` preview) — `board fetch
...
2b. Critical-path refresh (read-only toward ClickUp, writes only local
   ...
   wrap (live: in the `program_channel` post; shadow: in the `shadow_recipient` preview)
...
3. Build the wrap: `python3 scripts/daily_wrap.py
   ...
   in the wrap (live: in the `program_channel` post; shadow: in the `shadow_recipient`
   ...
   `program_channel` post; shadow: in the `shadow_recipient` preview) so the gap is visible
   ...
   gaps line (live: in the `program_channel` post; shadow: in the `shadow_recipient`
...
5. Milestone diamond: if `wrap["milestone"]["at_risk_paths"]` is non-empty, set
   ...
   in the `program_channel` post; shadow: in the `shadow_recipient` preview) — for each
   ...
   section entirely.
6. Wrap post (live audience: one message posted to `program_channel`; shadow audience:
   the same content as one `SHADOW (would post to <program_channel>): ...` message sent
   ...
   `program_channel` by name through the `message` tool; if that fails too, record
   `{program_channel_unresolved: true}` in state and, in `live`, send one fallback
   to every configured administrator with the full wrap content plus the literal
   blocker text `program channel unresolved`. If no administrator resolves, record
   the routing failure and send nothing. In `shadow`, include each would-be
   administrator fallback as `SHADOW (would DM <admin>): ...` in the
   `shadow_recipient` preview. Content:
...
7. Persist the risk register and local memory write-back in both `live` and
   ...
   `memory/daily/` overlay exists, append `memory/daily/$(date +%F).md` with sections
   `## Decisions`, `## Risks`
   ...
   a clean day. If `MEMORY.md` exists and a durable operating rule changed today,
   add a one-line entry following its existing style. An absent private memory
   overlay is valid and does not stop the rest of the workflow.
